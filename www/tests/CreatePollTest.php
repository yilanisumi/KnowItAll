<?php 
	...
	final class CreatePollTest extends TestCase{
		public function testPollSurveyIdLength(){
		...
		public function testPollSurveyIdPrefix(){
			include("database-connector.php");
			...
			$this->assertEquals("P", $surveyid[0]);
		}

		public function testPollSurveyIdValue(){
			include("database-connector.php");
		...
		public function testPollSurveyDbInsert(){
			include("database-connector.php");
			...
			$sql = $conn->prepare("SELECT * FROM survey WHERE survey_id LIKE \"P%\" ORDER BY survey_id DESC;");
			$sql->execute();
			$ans = $sql->get_result();
			...
			//delete the test row
			$sql = $conn->prepare("DELETE FROM survey WHERE survey_id LIKE \"P%\" ORDER BY survey_id DESC LIMIT 1;");
			$sql->execute(); 
		}
		public function testPollSurveyOptionDbInsert(){
			include("database-connector.php");

			// sample data for test rows
			$opts = array("testOption1", "testOption2", "testOption3");
			$surveyid = "P0000000001";
			
			// note how many rows were in the survey table before the test rows were inserted
			$sql = $conn->prepare("SELECT * FROM survey_options WHERE survey_id LIKE \"P%\" ORDER BY survey_id DESC;");
			$sql->execute();
			$ans = $sql->get_result();
			$numRowsBefore = $ans->num_rows;

			// $this->assertEquals(3, sizeof($opts));
			...
			// note how many rows were in the survey table after the test rows were inserted
			$sql = $conn->prepare("SELECT * FROM survey_options WHERE survey_id LIKE \"P%\" ORDER BY survey_id DESC;");
			$sql->execute();
			$ans = $sql->get_result();
			$numRowsAfter = $ans->num_rows;

			// check if the test rows were added to the survey_options table
			$this->assertEquals($numRowsBefore + 3, $numRowsAfter);

			// delete the test rows
			for($i = 0; $i < sizeof($opts); $i++){
				$sql = $conn->prepare("DELETE FROM survey_options WHERE survey_id = ? AND option_string = ?");
				$y = $opts[$i];
				$sql->bind_param('ss', $surveyid, $y);
				$sql->execute(); 
			}
		}

		public function testPollSurveyTrendingLimit() {
			include("database-connector.php");

			$surveyid = "P0000000099";

			// create the survey id (will work if previous test cases pass)
			$sql = $conn->prepare("SELECT survey_id FROM trending_survey WHERE survey_id LIKE \"P%\" ORDER BY survey_id DESC LIMIT 1;");
			$sql->execute();
			$ans = $sql->get_result();
			$maxid = $ans->fetch_assoc();
			$oldid = substr($maxid["survey_id"], 1);
		...
		public function testPollSurveyTrendingFifo() {
			include("database-connector.php");
			...
			$sql = $conn->prepare("SELECT survey_id FROM trending_survey WHERE survey_id LIKE \"P%\" ORDER BY survey_id ASC LIMIT 1;");
			$sql->execute();
			$ans = $sql->get_result();
